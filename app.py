...
def parent_dashboard():
    return render_template('parent_dashboard.html', username=g.user.username, children=g.user.children, profile_pic_url=g.user.profile_pic_url)
...
def child_dashboard():
    ...
    return render_template('child_dashboard.html', username=user.username, parent=parent_data, child_info=child_entry, profile_pic_url=user.profile_pic_url)
...
def update_location():
    if 'username' not in session: return jsonify(status='error', message="Not logged in")
    user = User.query.filter_by(username=session['username']).first()
...
def get_children_data():
    ...
    for child in parent_user.children:
        ...
        pic = child_user.profile_pic_url if child_user and child_user.profile_pic_url else url_for('static', filename='default-profile.png')
        children_list.append({
            'id': child.id, 'name': child.name, 'pairing_code': child.pairing_code,
            'last_latitude': child.last_latitude, 'last_longitude': child.last_longitude,
            'last_seen': child.last_seen.isoformat() if child.last_seen else None, 'profile_pic': pic
        })
